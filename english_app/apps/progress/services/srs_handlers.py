...
def handle_srs_post_request(request):
    ...
    if is_correct:
        ...
        if srs_obj.interval >= SRS_LEARNED_THRESHOLD:
            request.session['achievement_message'] = SRS_LEARNED_MSG.format(word.russian_name)
        else:
        ...
        request.session['error_message'] = SRS_WRONG_ANSWER_MSG.format(word.russian_name)
    return redirect('games:srs_technique')
