...
def srs_technique(request):
    ...
    srs_items = (
        UserSRS.objects
        .filter(user=request.user, word__status='PROCESS')
        .select_related('word')
    )
    ...
    return render(request, 'games/srs.html', {
...
def get_achievements(request):
    ...
    user_achievements = UserAchievement.objects.filter(user=request.user, achievement__in=achievements)
    ...
    return render(request, 'games/achievements.html',
                  {'achievements': achievements, 'user_achievements': user_achievements,
                   'user_achievement_names': user_achievement_names, 'selected': 'achievements',
                   'achievement_message': achievement_message,
